...
# List of columns we want to keep (List B)
keep_cols = [
    'GAME_ID', 'TEAM_ID', 'TEAM_ABBREVIATION', 'PLAYER_ID', 'PLAYER_NAME', 'START_POSITION',
    'FGM', 'FG3M', 'FG3A', 'FTM', 'PTS_3', 'PTS_2', 'PTS_FT',
    'E_NET_RATING', 'NET_RATING', 'AST_PCT', 'OREB_PCT', 'DREB_PCT', 'REB_PCT', 'TM_TOV_PCT',
    'USG_PCT', 'E_USG_PCT', 'E_PACE', 'PACE', 'POSS',
    'LOOSE_BALLS_RECOVERED', 'SCREEN_ASSISTS', 'TCHS', 'PTS_FB',
    'GAME_DATE', 'TEAM_NAME', 'TEAM_OFF_RATING', 'TEAM_DEF_RATING',
    'TEAM_E_OFF_RATING', 'TEAM_E_DEF_RATING', 'TEAM_AST_PCT', 'TEAM_REB_PCT',
    'TEAM_PACE', 'TEAM_E_PACE', 'TEAM_BLK', 'TEAM_STL', 'TEAM_PF'
]

# Keep only the allowed columns
cleaned_df = merged_df[keep_cols]

# Save to final CSV
cleaned_df.to_csv("merged_player_team_adv_stats_2024-25.csv", index=False)
print("Merged player + team stats saved.")
print(f"[✓] Final dataset saved with {len(cleaned_df)} rows and {len(cleaned_df.columns)} columns.")
